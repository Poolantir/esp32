...
static void handleCommand(const String& raw) {
  ...
  if (command == "MODE") {
    if (doc["action"].isNull()) {
      logParsed("MODE <missing action>");
      return;
    }
    logParsed(String("MODE ") + (type.length() ? type : String("<any>")) + " " + actionText);
    if (actionText != "TEST" && actionText != "SIM") {
      logParsed("MODE invalid action (expected TEST or SIM)");
      return;
    }
    setMode(actionText);
    logParsed(String("MODE applied -> ") + modeName(gMode));
    return;
  }
